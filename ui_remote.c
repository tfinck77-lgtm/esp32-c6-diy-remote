...
// ---------------------------------------------------------------------
// Bluray-Player, Internetradio, LED-Kerzen, Nixietube-Uhr: gemeinsame
// Kachel-Definition. data/nbits sind 1:1 der aufgezeichnete Raw-Data-Wert
// samt Bitanzahl aus den IR-Codes-*.txt-Dateien - siehe ir_send_generic()
// in ir_bsp.cpp fuer den Versand.
// ---------------------------------------------------------------------
typedef struct {
    const char *caption;
    const char *icon; // NULL -> nur Text, zentriert (z.B. Zifferntasten)
...
static const soundbar_tile_def_t soundbar_tiles[] = {
    { "Ein/Aus",     LV_SYMBOL_POWER,      0 },
    { "Mute",        LV_SYMBOL_MUTE,       1 },
    { "TV",          NULL,                 2 },
    { "HDMI",        NULL,                 3 },
    { "OPT",         NULL,                 4 },
    { "AUX",         NULL,                 5 },
    { "Pure",        NULL,                 6 },
    { "Bluetooth",   LV_SYMBOL_BLUETOOTH,  7 },
    { "Vol +",       LV_SYMBOL_VOLUME_MAX, 8 },
    { "Bass +",      LV_SYMBOL_PLUS,       9 },
    { "Vol -",       LV_SYMBOL_VOLUME_MID, 10 },
    { "Bass -",      LV_SYMBOL_MINUS,      11 },
    { "Movie",       NULL,                 12 },
    { "Night",       NULL,                 13 },
    { "Music",       NULL,                 14 },
    { "Dialog\nLow", NULL,                 15 },
...
static void soundbar_tile_event_cb(lv_event_t *e)
{
    uint32_t index = (uint32_t)(uintptr_t)lv_event_get_user_data(e);
    lv_obj_t *tile = lv_event_get_target(e);

    ir_send_soundbar_command(soundbar_tiles[index].command);
...
static void ui_show_soundbar_remote(void)
{
    ...
    for (uint32_t i = 0; i < SOUNDBAR_TILE_COUNT; i++) {
        uint32_t col = i % REMOTE_COLS;
        uint32_t row = i / REMOTE_COLS;
        const soundbar_tile_def_t *t = &soundbar_tiles[i];
        ...
        lv_obj_add_event_cb(tile, soundbar_tile_event_cb, LV_EVENT_CLICKED,
                            (void *)(uintptr_t)i);
    }
}
